...
class FolderModelForm(BootstrapForm,forms.ModelForm):
    ...
    def clean_name(self):
        ...
        # models.FileRepository.objects.filter(file_type=2,name=name,project=request.tracer.project,parent__isnull=True)
        # models.FileRepository.objects.filter(file_type=2, name=name, project=request.tracer.project,parent=父目录对象)
...
class FileModelForm(forms.ModelForm):
    ...
    #如果不考虑校验etag等的数据的话，可以把这个clean方法去掉
    """
    def clean(self):
