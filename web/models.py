...
class Issues(models.Model):
    ...
    model = models.SmallIntegerField(verbose_name='模式',choices=model_choices,default=1)
    #on_delete=models.SET_NULL代表关联内容删除时，不删除这条内容，仅仅是把外键置空，与on_delete=models.CASCADE不同
    parent = models.ForeignKey(verbose_name='父问题',to='self',related_name='child',on_delete=models.SET_NULL,null=True,blank=True)

    creator = models.ForeignKey(verbose_name='创建者',to='UserInfo',on_delete=models.CASCADE,related_name='create_problems')
    create_datetime = models.DateTimeField(verbose_name='创建时间',auto_now_add=True)
    lastest_update_datetime = models.DateTimeField(verbose_name='最后更新时间',auto_now=True)
...
class ProjectInvite(models.Model):
    ...
    use_count = models.PositiveIntegerField(verbose_name="已邀请数量",default=0,null=True,blank=True)
    period_choices = (
        (30,'30分钟'),
        (60, '1小时'),
        (300, '5小时'),
